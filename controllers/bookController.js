...
exports.book_list = asyncHandler(async (req, res, next) => {
  ...
  res.render('pages/book_list', { title: 'Book List', book_list: allBooks });
});
...
exports.book_create_post = [
  ...
  body('title', 'Title mist not be empty.')
    .trim()
    .isLength({ min: 1 })
    .escape(),
...
exports.book_update_post = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      // There are errors. Render form again with sanitized values/eoor messages
      // Get all authors and genres for form
      const [allAuthors, allGenres] = await Promise.all([
        Author.find().exec(),
        Genre.find().exec(),
